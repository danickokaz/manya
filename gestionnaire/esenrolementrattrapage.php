...
if(isset($_SESSION['jgm']) and !empty($_SESSION['jgm'])){
    ...
    if(isset($_GET['classe'],$_GET['salle'],$_GET['annee_academique']) and !empty($_GET['classe']) and !empty($_GET['salle']) and !empty($_GET['annee_academique'])){
        ...
        if($req->rowCount()==1){
            ...
            $req->execute([$classe,$salle,$annee_academique,7]);
            ...
            if($req->rowCount()!=0){
                ...
                $html .= '<h1 style="font-family:Verdana;text-align:center;">LISTE DES ÉTUDIANTS EN ORDRE AVEC L\'ENROLEMENT DE RATTRAPAGE POUR L\'ANNÉE ACADÉMIQUE '.$libelle_annee_academique.'</h1>';
