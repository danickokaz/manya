...
if(isset($_SESSION['jgm']) and !empty($_SESSION['jgm'])){
    ...
    if(isset($_GET['inscription']) and !empty($_GET['inscription'])){
        $inscription = htmlspecialchars($_GET['inscription']);
        ...
        if($req->rowCount()==1){
            ...
            LEFT JOIN genre g ON g.id = e.genre WHERE e.id_etat_etudiant=? AND e.id_inscription=? ORDER BY e.nom,e.postnom,e.prenom ASC");

            $req->execute([1,$inscription]);
